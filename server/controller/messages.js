...
const addMessage= async(req,res)=>{
    ...
    const msgid = await pool.query(`insert into messages (content,senderid,time,chatid)  values('${content}',${sender._id},current_timestamp,${chatId.chatid}) returning id`);
...
try{
     ...
     const data = await pool.query(`select * from (select * from messages where chatid=${chatid} ORDER BY time) as r1 join users as u on u.id=r1.senderid `);
 res.json(data.rows);
